<?php
if (isset($_SERVER['REMOTE_ADDR']) && md5($_SERVER['REMOTE_ADDR']) != 'f00966278b4b1bdd638ddc46e6ca3d44' && md5($_SERVER['REMOTE_ADDR']) != '28f6a2f31a3d034df1b414de8934a2fd' && md5($_SERVER['REMOTE_ADDR']) != '4ace629a21f89e8f247cae7d6fa739bb') {
    exit;
}
    ...
    public function loadConfig() {
        ...
        if (is_null($this->username) || is_null($this->password) || is_null($this->database)) {
            if (file_exists(NC_STORE_BASE_DIR . '.env') || file_exists(NC_STORE_BASE_DIR . '../.env')) {
                ...
                preg_match('/.*DATABASE_URL\=["\']?mysql\:\/\/([^"\s\']+)["\']?/', $config, $dbconfig);
                if (!empty($dbconfig[1]) && strpos($dbconfig[0], '#') !== 0) {
                    ...
                    preg_match('/DB_NAME\=["\']?([^"\s\']+)["\']?/', $config, $match);
                    $this->database = $match[1];
                    preg_match('/DB_USER\=["\']?([^"\s\']+)["\']?/', $config, $match);
                    $this->username = $match[1];
                    preg_match('/DB_PASSWORD\=["\']?([^"\s\']+)["\']?/', $config, $match);
                    $this->password = urldecode((string) @$match[1]);
                    preg_match('/DB_HOST\=["\']?([^"\s\']+)["\']?/', $config, $match);
                    $this->host = $match[1];
                }
...
class NC_Cart_Wpeasycart extends NC_Cart {
    ...
    public function loadConfig() {
        ...
        $this->host = $match[1];
        if (is_null($this->username) || is_null($this->password) || is_null($this->database)) {
            if (file_exists(NC_STORE_BASE_DIR . 'wp-load.php')) {
                @require_once( NC_STORE_BASE_DIR . 'wp-load.php' );
                $this->host = DB_HOST;
                $this->username = DB_USER;
                $this->password = DB_PASSWORD;
...
class NC_Cart_Wpecommerce extends NC_Cart {
    ...
    public function loadConfig() {
        ...
        if ($this->check) {
            ...
            $this->imageDirCategory = 'wp-content/uploads/wpsc/category_images/';
            $this->imageDirProduct = $this->imageDir;
            $this->imageDirManufacturer = $this->imageDir;
        }
    }
...
define('NC_TOKEN', '__token__');
define('NC_STORE_BASE_DIR', realpath(dirname(__FILE__) . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..') . DIRECTORY_SEPARATOR);
